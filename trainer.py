...
class Trainer:
    ...
    def _build_distributed_loaders(self, cfg: dict):
        ...
        # num_workers: saturate IO without over-subscribing CPU
        # 4 workers per GPU is a solid default on RunPod instances
        num_workers = min(d_cfg.get("num_workers", 4), 4) * max(1, self.world_size // 4)
        ...
        for epoch in range(self.epoch, num_epochs):
            ...
            if self.is_main:
                ...
                logger.info(
                    ...
                    + " | ".join(
                        f"{k}={v:.4f}" for k, v in val_logs.items() if k != "recon_mse"
                    )
                )
