...
class BridgeInference:
    ...
    def __call__(
        ...
        returns: (B, 4T, output_dim)  float32  — always on CPU
        """
        if tokens.dim() == 2:
            tokens = tokens.unsqueeze(0)

        tokens = tokens.to(self.device)
        features, _ = self.model(tokens)           # (B, 4T, output_dim)  — may be fp16

        # Always return float32 to the caller
        features = features.float()

        if mask is not None:
            # Zero-out padded positions: mask is (B, T_m), expand to (B, 4T, 1)
            mask4 = mask.repeat_interleave(4, dim=-1).to(self.device)   # (B, 4T)
            features = features * mask4.unsqueeze(-1)
    ...
    def from_audio(self, audio_path: str) -> torch.Tensor:
        ...
        tokens = extractor.extract(waveform, 16000)   # (T, num_codebooks)

        features = self(tokens)                        # (1, 4T, output_dim)
        return features.squeeze(0)                     # (4T, output_dim)
...
class StreamingBridgeInference:
    ...
    Usage:
        ...
        for mimi_chunk in token_stream:           # (chunk_size, num_codebooks)
            feat_chunk = stream.step(mimi_chunk)  # (4*chunk_size, output_dim)
    """
    ...
    def step(self, tokens_chunk: torch.Tensor) -> torch.Tensor:
        ...
        returns      : (4C, output_dim)  float32  — on CPU
        """
        ...
        self._step_count += 1
        return features.squeeze(0).float().cpu()   # (4C, output_dim)
    ...
    def stream_tokens(self, tokens: torch.Tensor) -> Iterator[torch.Tensor]:
        ...
        yields : (4*chunk_size, output_dim) tensors
        """
        self.reset()
        T = tokens.shape[0]
...
def benchmark_streaming(
    ...
    print(f"Chunk size  : {chunk_size} Mimi frames → {chunk_size * 4} feature frames")
...
def main():
    ...
    def get_tokens(cfg: dict) -> torch.Tensor:
        ...
        return MimiExtractor(cfg["paths"]["mimi_model"]).extract(wav, 16000)
